...
class AlphaEnsemble:
    ...
    def _get_returns_matrix(self) -> pd.DataFrame:
        """
        Helper to get a DataFrame of daily returns for all strategies.

        Column names use ``<indicator>_<job_id>`` for debuggability.
        Integer indices were previously used, making it hard to identify
        which column belonged to which strategy during inspection.
        """
        returns_dict = {}
        for r in self.results:
            # Descriptive key: indicator name + short job id (unique within a batch)
            col_key = f"{r.job.indicator_name}_{r.job.id}"
            returns_dict[col_key] = r.equity_curve.pct_change().fillna(0)
